...
export default class Calendar extends React.Component {
  ...
  renderDates(eventsEachDay) {
    ...
    var padDays =
      (((-this.state.current.daysInMonth() - this.state.current.day()) % 7) +
        7) %
      7; //number of days to fill out the last row
    ...
    return [
      ...
      days.map((x) => {
        if (
          ...
          this.state.current.isSame(this.state.today, "month")
        ) {
          return (
            <div
              className="day"
  ...
  getRenderEvents(events, singleEvents) {
    let eventsEachDay = [
      ...Array(this.state.current.daysInMonth()),
    ].map((e) => []); //create array of empty arrays of length daysInMonth
    ...
    events.forEach((event) => {
      if (event.recurrence) {
        ...
        let dates = Calendar.getDatesFromRRule(
          event.recurrence[0],
          event.startTime,
          moment(this.state.current).subtract(duration),
          moment(this.state.current).add(1, "month")
        );
    ...
    singleEvents.forEach((event) => {
      if (event.recurrence) {
        ...
        this.renderSingleEvent(eventsEachDay, moment(event.startTime).date(), {
          ...event,
          ...eventProps,
        });
      }
  ...
  drawMultiEvent(eventsEachDay, props) {
    ...
    while (curDate.isSameOrBefore(endDate, "day")) {
      if (
        curDate.date() == this.state.current.daysInMonth() &&
        !endDate.isSame(this.state.current, "month")
      ) {
        if (this.props.showArrow) {
          arrowRight = true;
        }
  ...
  static getDatesFromRRule(str, eventStart, betweenStart, betweenEnd) {
    //get recurrences using RRule
    let rstr = `DTSTART:${moment(eventStart)
      .utc(true)
        ...
        {this.state.showFooter && (
          <div className="calendar-footer">
            ...
            >
              ...
              {moment().tz(this.state.userTimezone).format("z")})
            </div>
            <div
              css={css`
                vertical-align: top;
